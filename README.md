...
The *best* case for insertion sort is an input that is already sorted, taking **(O(*n*)) time, where *n* is the number of items in the list.

The *worst* case is an array sorted in reverse order, which forces the algorithm to shift the current value to the front of the sorted sub-list at each iteration. This takes O(*n*<sup>2</sup>) time.
